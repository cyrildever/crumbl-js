...
export class Uncrumbl {
    ...
    private async doUncrumbl(): Promise<Buffer> {
        ...
        // 2- Decrypt crumbs
        let uncrumbs = new Map<number, Uncrumb>()
        ...
        if (hasAllCrumbs) {
            ...
            // 6a- Check
            if (!collector.check(Buffer.from(deobfuscated))) {
                logger.log('source has not checked verification hash', WARNING) // TODO Change it as an error?
            }
            ...
            for (let i = 0; i < indexSet.size; i++) {
                if (uncrumbs.has(i)) {
                    partialUncrumbs += uncrumbs.get(i)!.toString()
                }
            }
