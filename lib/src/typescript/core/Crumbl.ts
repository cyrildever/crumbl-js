...
export class Crumbl {
    ...
    private async doCrumbl(): Promise<string> {
        ...
        for (let i = 1; i < allocation.size + 1; i++) { // Initial index is 1 in allocation
            if (allocation.has(i)) {
                const allocated = allocation.get(i)!
                for (let j = 0; j < allocated.length; j++) {
                    const crumb = await encrypt(slices[i], i, allocated[j])
                    crumbs.push(crumb)
                }
            }
        }
        ...
        const hSrc = hash(this.source, DEFAULT_HASH_ENGINE)

        // 5- Finalize the output string
        let stringifiedCrumbs = new Array<string>()
