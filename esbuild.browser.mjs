...
// modules out of the box, but the library (and its dependencies) genuinely
// rely on a few of them in the browser:
//   - `crypto`           -> hashing in ecies-geth / feistel-cipher
...
//
// Note: `buffer` is intentionally left out of the globals polyfill. feistel-cipher
// (a dependency) installs its own browser Buffer (`Buffer = require('buffer/').Buffer`
// guarded by `typeof window`), and injecting a second Buffer global would clash
...
await build({
  ...
  platform: 'browser',
  format: 'iife',
  // feistel-cipher installs its own browser Buffer via a bare global assignment
